...
3. 点击“停止并导出 CLI 配置”。应用会先停止本地任务，等待已发出的选退课请求核对结果，再验证登录：有门户会话时续期并校验身份，否则只校验 Token。失败时不生成配置，成功时导出最新凭据。
...
Windows 网页登录生成的完整抢课配置还包含可选的 `portalSession`（学校门户的 `SESSION` 和 `cookie` 会话凭据）。CLI 在任务启动时和每 5 分钟使用它换取新 Token，并验证学生身份；等待开始、重试退避和轮询间隔也会续期。新 Token 仅在当前 CLI 进程内保存，不写入运行日志或配置文件。门户会话失效、续期失败或身份改变时立即停止，需要重新网页登录并导出配置。

没有 `portalSession` 的配置每 5 分钟只执行身份与保活检查，不能延长 Token 的有效期。`--check` 及只读 `--action` 不执行续期；只读验证命令导出的配置不包含门户会话。导出配置包含登录凭据，不能公开分享。

门户会话与选课 Token 的有效期互相独立。学校通过 `Session-Invalid: true` 响应头明确报告门户会话失效，此时即使 HTTP 状态是 200、选课 Token 尚未到期，也无法续期。请重新完成学校网页登录，导出新配置并及时启动 CLI；仅再次复制原配置不会刷新门户会话。没有该标记的空响应或格式异常会单独报错，不会判定为会话过期。
