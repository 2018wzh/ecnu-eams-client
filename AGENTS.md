...
- The portal can report expired sessions as HTTP 200 with an empty body and `Session-Invalid: true`. Check this header before decoding JSON; do not classify unmarked empty/malformed responses or arbitrary business errors as expired sessions. Portal session validity is independent of the course-selection JWT expiry.
- Full GUI automation export stops local tasks, reconciles outstanding actions, then verifies the session through shared keepAlive before exporting the latest credentials. A failed check must prevent export. Read-only exports remain unchanged and never renew.
